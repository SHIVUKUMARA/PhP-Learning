...
class Omni_publish extends CI_Controller
{
    ...
    public function publish()
    {
        ...
        $ecommerce = $this->input->post('ecommerce_platforms') ?? [];
        ...
        if (empty($ecommerce) && empty($social)) {
        ...
        $crudPayload = [
            ...
            'published_at'        => date('c'),
            'published_from'      => 'omni_panel'
        ];

        $responses = [];

        // CrudCrud (FULL PAYLOAD)
        if (in_array('CrudCrud', $social, true)) {
            $crudCrudUrl =
                'https://crudcrud.com/api/2350da2f06394fc792347ab832c93d08/products';

            $responses['crudcrud'] =
                $this->_sendPostRequest($crudCrudUrl, $crudPayload);
        }

        // Mock API 
        if (in_array('Mock API', $social, true)) {
            $mockApiUrl = 'https://6968d5a969178471522bae41.mockapi.io/api/products';

            $responses['mockapi'] =
                $this->_sendPostRequest($mockApiUrl, $crudPayload);
        }

        // Beeceptor (FULL PAYLOAD)
        if (in_array('Beeceptor', $social, true)) {
            $beeceptorUrl = 'https://ca88fe7b23e3b6cf0603.free.beeceptor.com/api/products';
            $apiResp = $this->_sendPostRequest($beeceptorUrl, $crudPayload);

            // unwrap array if response is numeric array
            $responses['beeceptor'] = $apiResp['response'] ?? [];
        }
        // Supabase (FULL PAYLOAD)
        if (in_array('Supabase', $social, true)) {
            ...
            $supabasePayload = $crudPayload;
            unset($supabasePayload['id']);
            unset($supabasePayload['product_id']);
            unset($supabasePayload['published_at']);
            unset($supabasePayload['published_from']);
            ...
            curl_setopt_array($ch, [
                ...
                CURLOPT_POST            => true,
                CURLOPT_POSTFIELDS      => json_encode($supabasePayload),
                CURLOPT_TIMEOUT         => 30
            ]);
    ...
    public function get_published_products()
    {
        ...
        if ($platform === 'Supabase') {
            ...
            if ($error) return $this->_json(false, 'Supabase Curl error: ' . $error);
